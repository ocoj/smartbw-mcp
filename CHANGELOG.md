...
- **空判定与取值统一走同一原子快照（复核 E-3 / R-3.5）**: `smart_search._do_fuzzy_search()` 原先以 `_ensure_items()` 的返回值做空判定，随后又读取 `_snapshot_items()`——两次读取可能落在不同代（无害，但属冗余）。现保留 `_ensure_items()` 的装载/刷新副作用，空判定与取值统一由 `_snapshot_items()` 提供。语义等价：无数据时两者同为空、装载失败时同回退旧缓存，仅消除代际窗口

### 🧪 测试

- 离线：`52 passed / 2 skipped`；`ruff check .` 全绿；`smart_search.py` 未覆盖行数保持不变（264），新增语句已被既有用例覆盖
...
> 依据第三方复核报告的收尾修复（复核报告为内部文档，不随仓库发布）。
...
> 依据 v2.3.1 第三方审计报告的修复提交（审计报告为内部文档，不随仓库发布）。
